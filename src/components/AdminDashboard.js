...
case 'sitePerformance':
  ...
  const handleCropConfirm = async () => {
    try {
      ...
      setTeamMemberForm((prev) => ({ ...prev, image: file }));
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div className="bg-white rounded-lg p-6 w-full max-w-lg relative">
              ...
              <div className="flex justify-end gap-2 mt-4">
                <button onClick={() => { setShowCropModal(false); setCroppingImage(null); }} className="px-4 py-2 bg-gray-300 rounded">Cancel</button>
                <button onClick={handleCropConfirm} className="px-4 py-2 bg-orange-500 text-white rounded">Crop & Save</button>
              </div>
            </div>
          </div>
        )}
