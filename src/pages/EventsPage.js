...
const mockEvents = [
  {
    id: 1,
    title: "Feast of St. Enrico",
    date: new Date(2023, 9, 31), // October 31, 2023
    time: "3:00AM",
    description: "Para kay St. Enrico",
    venue: "Jan lang sa tabi",
    tags: ["UC InTTO", "TARAKI"],
    type: "upcoming"
  },
  {
    id: 2,
    title: "Angel Investing 101 Pre-Launch Dinner",
    date: new Date(2025, 5, 6), // November 3, 2023
    time: "5:00 PM - 8:00 PM",
    description: "An intimate gathering of visiounary investors and ecosystems leaders to introduce Angel Investing 101 - a foundational program designed to equip aspiring investors with the knowledge, tools, and connections to confidently invest in startups. This pre-launch dinner sets the stage for collaboration, insight-sharing, and early access to the program's opportunities.",
    venue: "Fullsuit PH Pod",
    tags: ["TARAKI"],
    type: "upcoming",
    rsvpLink: "https://example.com/rsvp-angel-investing"
  },
  {
    id: 3,
    ...
    time: "10:00AM - 2:00PM",
    description: "A past event for demonstration purposes.",
...
function EventsPage() {
  ...
  const filteredEvents = mockEvents.filter(e =>
    (tab === "upcoming" ? e.type === "upcoming" : e.type === "past") &&
    (filter === "All Events" || e.tags.includes(filter))
  );
          ...
          <div className="flex-[1_1_0%] md:pr-12">
            <div className="flex flex-col sm:flex-row items-center gap-2 mb-8">
              ...
              >
                ...
                <option>BSU ATBU</option>
                <option>UC InTTO</option>
                <option>SLU ConRes</option>
                <option>UPB SILBI</option>
                <option>IFSU IPTBM</option>
              </select>
          ...
          <div className="bg-white rounded-xl shadow-lg max-w-6xl w-full p-6 relative animate-fadeIn flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1 pr-4">
              ...
              <div className="flex items-center gap-2 mb-2 text-gray-500 text-sm">
                ...
                <span className="font-bold text-black mr-1">{selectedEvent.date.toLocaleDateString(undefined, { weekday: 'long' })}</span>
                <span className="mr-1">{selectedEvent.date.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              {Array.isArray(eventImages[selectedEvent.id]) && eventImages[selectedEvent.id].length > 0 ? (
                <div className="relative w-full flex flex-col items-center">
                  <img
                    src={eventImages[selectedEvent.id][galleryIndex]}
                    alt={`Event ${galleryIndex + 1}`}
                    className="w-full max-w-xs h-64 object-cover rounded-lg shadow-md mb-2"
                  />
                  {eventImages[selectedEvent.id].length > 1 && (
                    <div className="flex items-center justify-center gap-4 mt-2">
                      <button
                        className="px-3 py-1 bg-gray-200 rounded hover:bg-orange-200 text-gray-700 font-bold text-lg"
                        onClick={() => setGalleryIndex((galleryIndex - 1 + eventImages[selectedEvent.id].length) % eventImages[selectedEvent.id].length)}
                        aria-label="Previous image"
                      >
                        &#8592;
                      </button>
                      <span className="text-sm text-gray-500">{galleryIndex + 1} / {eventImages[selectedEvent.id].length}</span>
                      <button
                        className="px-3 py-1 bg-gray-200 rounded hover:bg-orange-200 text-gray-700 font-bold text-lg"
                        onClick={() => setGalleryIndex((galleryIndex + 1) % eventImages[selectedEvent.id].length)}
